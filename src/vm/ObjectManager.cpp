				...
				*/
				...
				void GC() {
					...
					for(int i=0; i<Scopes.size(); i++) {
						...
						InitUnscannedScope(scope, unscanned);	//把作用域里的变量（也就是GCRoots）加载到unscanned里
						MarkScopeObject(scope, unscanned);	//遍历该作用域的变量涉及到的全部对象，并且标记他们
					}
				...
				void InitUnscannedScope(ObjectScope &scope, ArrayList<datatype::DataType*> &unscanned) {
				...
				void MarkScopeObject(ObjectScope& scope, ArrayList<datatype::DataType*>& unscanned) {
